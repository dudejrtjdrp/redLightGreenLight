...
- 탈락자는 **들고 있던 짐을 길에 흘리면서** 술래 왼쪽으로 걸어가 줄을 선다(원작 연출).
  흘린 짐은 트랙에 남아 뒤 무버가 회수 가능(라운드 중 낙하는 술래 가점, 종료 후엔 무가점).
- **90초(1분 30초)** 경과 시 **완주 못 한 생존자도 그대로 탈락 처리** 후 정산 + 결과 화면.
  (전원 완주/탈락 시 조기 종료)
- 빈 턴 반환: RED 동안 아무 활동(이동/낙하)이 없으면 그 시간을 라운드 예산에 되돌려줌.

## 점수

- **무버** = 반입 짐 × 1 + 생존보너스(`K / 생존자수`). 타임업 탈락 도입으로 생존자=완주자. 탈락자는 0.
- **술래** = (낙하 수확 × 1 + 잡기 1명당 0.8점) × **인원수 보정 `(4/무버수)^1.3`**.
  - 잡기의 실질 가치 = 직접 보너스 + 잡힌 무버가 흘리는 짐 수확(줄서기 낙하).
  - Phase 10: 잡기 보너스를 가속형 `c(c+1)/2` → **선형**으로 변경(가속형은 다인전에서 술래 점수가
    제곱으로 커져 인원별 밸런스를 깨는 주범이었음 — 시뮬로 확인).
...
| **생존보너스 K** | **20** | GameBalance.score | Phase 11: 타임업 탈락 반영 재튜닝 |
| **낙하 점수 배율** | **1.0** | GameBalance.score | "낙하 1개=술래 1점" 대칭 |
| **잡기 보너스** | **선형 0.8/명** | GameBalance.score | Phase 11: 줄서기 낙하가 잡기 가치 대체 |
| **술래 인원수 보정** | **(4/무버수)^1.3** | GameBalance.score.seekerCountNorm | Phase 11: power 1→1.3 |
| **되잡기 조작력 연동** | **couple 0.8** | BalanceConfig.counterLeanCouple | Phase 10: 고속·종반 조작감 개선 |
...
- **발견 1**: 술래 점수원(낙하·잡기)은 인원수에 비례해 커지지만 1등 무버 점수는 인원과 무관
  → 단일 배율로는 인원별 밸런스 불가 → **인원수 보정 `×(4/무버수)` 도입**.
- **발견 2**: 가속형 잡기 보너스(`c(c+1)/2`)는 다인전에서 제곱으로 커져 여전히 편차를 만듦
  → **선형(1명당 고정점)으로 변경**하니 전 구간이 수렴.
- **발견 3 (Phase 11)**: 줄서기 낙하(잡힌 무버의 짐 수확) + 타임업 탈락 규칙 추가로 분포가
  다시 변함 → 재시뮬로 K=20 / 잡기 0.8 / 보정 power 1.3 재수렴.
- **결과** (Phase 11 — 실제 정산 코드로 600라운드×5구간):

| 무버 수 | 술래 승률 | 플레이어 승률 | 평균 점수(술래 vs 1등) |
|---|---|---|---|
| 2 (봇1) | 52.2% | 47.8% | 16.2 vs 12.8 |
| 3 (봇2) | 47.7% | 52.3% | 17.0 vs 14.5 |
| 4 (봇3) | 46.7% | 53.2% | 16.5 vs 14.4 |
| 5 (봇4) | 51.5% | 48.5% | 16.4 vs 14.6 |
| 7 (봇6) | 47.8% | 52.2% | 15.9 vs 15.3 |
...
- [x] Phase 10: 탈락자 술래 옆 줄서기 연출 + 균형 조작감 개선(counterLeanCouple) + 잡기 보너스 선형화 재밸런스
- [x] Phase 11: 탈락자 짐 흘리기(회수 경제) + 타임업 미완주 탈락 + 낙하 짐 시각 통일(도로 상판 정렬·크기 일치) + 재밸런스
