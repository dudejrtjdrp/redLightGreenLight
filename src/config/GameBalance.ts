...
export const GameBalance = {
  ...
  score: {
     ...
     * Phase 11 튜닝(줄서기 낙하 + 타임업 탈락 규칙 반영 재시뮬): 20.
     * 타임업 탈락 도입으로 생존자=완주자만 남아 K 가치 상승 → 재조정.
     */
    survivorBonusK: 20,
    /**
     * 낙하 짐 1개당 술래 점수 배율(최종 정산에만 적용).
     * "낙하 1개 = 술래 1점 = 무버 -1점" 대칭. 잡힌 무버의 짐도 길에 흘리므로(줄서기)
     * 잡기의 실질 가치에 낙하 수확이 포함된다.
     */
    dropPointScale: 1.0,
    /**
     * 잡기 보너스 배율(선형: 잡기 1명당 catchBonusScale점).
     * Phase 11: 1.8 → 0.8. 잡힌 무버가 짐을 흘리게 되면서(낙하 수확이 잡기 가치를
     * 대체) 직접 보너스는 낮춰 이중 보상 방지.
     */
    catchBonusScale: 0.8,
    /** 잡기 보너스 방식: "linear"(c × scale) | "accelerating"(c(c+1)/2 × scale). */
    catchBonusMode: "linear" as "linear" | "accelerating",
    /**
     ...
     * Phase 11 재검증(줄서기 낙하·타임업 탈락 반영): power 1 → 1.3.
     * 잡힌 무버의 짐 낙하가 인원수에 비례해 커져 보정도 더 세게. 전 구간 46~49%.
     */
    seekerCountNorm: {
      ref: 4,
      power: 1.3,
